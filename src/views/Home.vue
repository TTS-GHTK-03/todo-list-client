<template>
  ...
  <section class="min-h-screen bg-gradient-to-b from-[#273758] to-[#0e1e42]">
    <section class="flex items-center justify-between h-[100vh] pt-[10vh] gap-4">
      <div class="pl-20 flex flex-col space-y-3 font-charlie-display text-white ml-">
        ...
        <button class="w-24 h-12 font-apple text-base font-medium bg-[#ffab00] border rounded border-transparent text-black hover:bg-[#e69a00]">
          Get it free
        </button>
      </div>
  ...
  <section class="h-screen flex flex-col items-center pt-8 relative overflow-hidden bg-[#0e1e42]">
    <div class="absolute inset-0 z-0">
      <div class="absolute inset-0 bg-gradient-to-br from-[#0e1e42] to-[#1e46a4]" style="clip-path: polygon(66% 100%, 90% 90%,100% 50%, 100% 100%);"></div>
    </div>

    <!-- <div class="absolute inset-0 bg-gradient-to-br from-[#0e1e42] to-[#1e46a4] z-0" style="clip-path: polygon(75% 0, 100% 0, 100% 25%, 100% 100%, 0 100%);"></div> -->
    ...
    <div class="flex space-x-2 z-10">
      ...
      >
        {{ tab.label }}
        <span class="absolute inset-0 border-2 border-transparent rounded-full hover:border-white transition-colors duration-300"></span>
      </button>
    </div>
      ...
      <div class="flex justify-between gap-6 mt-10" :key="activeTab">
        <div class="w-[450px] text-white font-charlie-display ml-8 mt-10">
          ...
          <div class="py-4 mt-12 rounded-r-full flex flex-col space-y-2 bg-gradient-to-r from-[#0e1e42] to-[#2b64fb]">
            ...
            <p class="text-white text-lg pl-2 font-medium font-apple w-[400px]">{{ tabs[activeTab].credentials.text }}</p>
    ...
    <div class="grid grid-cols-2 h-full">
      <div class="flex items-center justify-center bg-gradient-to-b from-[#0e1e42] to-[#1b2b4d]">
        <div class="flex flex-col  text-center mt-72 ">
          <h1 class="max-w-4/5 text-2xl font-semibold font-charlie-display text-white mb-4">Templates give you a head start</h1>
          ...
          <div class="flex flex-col space-y-4 mt-8 border-2 border-transparent">
            <div v-for="template in templateList" :key="template.name" class="  min-h-24 min-w-[500px] flex items-center justify-center bg-[#273758] m-0 rounded-md">
              <div class="grid grid-cols-[100px,1fr] w-full h-full items-center">
                ...
                <div class="bg-[#1b2b4e]  h-[100px] ">
                  <div class="flex flex-col items-start h-full ml-8 ">
                    <span class="text-white text-xl font-semibold font-charlie-display mt-6 mb-2 h-full ">{{ template.name }}</span>
                    <span class="text-white text-base font-charlie mb-2 h-full flex z-10 hover:underline cursor-pointer ">
                      Learn more
      ...
      <div class="bg-[#0e1e43] h-full flex items-center justify-center">
        <div class="flex flex-col s pace-y-2 items-center text-center mt-72">
          ...
          <p class="w-4/5  text-white font-charlie-display mb-8">Jira adapts to the way you work, not the other way around. Start simple, customize as you go. </p>
          <img src="../assets/img/workflow_img.png" alt="" class ='w-2/3'>
        </div>
      </div>
    </div>

    <h1 class="text-4xl font-semibold text-center absolute inset-0 mt-10 font-charlie-display justify-center text-white">Easier than ever</h1>
...
@font-face {
  ...
  src: url('../assets/fonts/charlie-display.woff2') format('woff2'),
       url('../assets/fonts/charlie-display.woff') format('woff');
  font-weight: normal;
  font-style: normal;
}

.font-charlie-display {
