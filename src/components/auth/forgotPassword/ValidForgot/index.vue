  ...
  <div
    ...
    <div class="flex flex-col items-center w-full font-apple select-text">
      ...
      <form @submit.prevent="handleValidate" class="flex flex-col items-center mt-4 w-full">
        <span class="list-none text-sm mb-0 font-apple w-3/4">To complete your reset password, enter the code we've sent
          to:</span>
        ...
        <div class="flex justify-between w-4/5 mt-4">
          <input v-for="(digit, index) in code" :key="index" type="text" maxlength="1" class="code-input"
            @input="moveToNext($event, index)" @keydown.backspace="moveToPrev($event, index)" v-model="digit.value"
            required />
        </div>
      ...
      <div class="flex items-center mt-4 mb-0 space-x-2">
        <span class="list-none text-[#2f66e6] text-sm hover:underline cursor-pointer mb-0">Didn't receive an email?
          Resend email</span>
      </div>
      ...
      <footer class="w-full flex flex-col items-center mt-4 space-y-2">
        ...
        <ul class="flex items-center justify-center space-x-2">
          <li class="list-none text-xs hover:underline cursor-pointer">Login help</li>
