      ...
      <div class="h-[480px] overflow-y-auto mt-4">
        ...
        >
          <div class="rounded flex flex-col mt-4">
            <div
              class="flex justify-between font-apple text-text-dark-thin text-sm"
            >
              <div class="font-semibold flex items-center">
                <div class="w-5 h-5 p-1 flex items-center ml-4">
                  <input
                    type="checkbox"
                    id="checkbox"
                    class="h-3 w-3 border-gray-300 rounded"
                  />
                </div>
                <button @click="toggleSprint(sprint.id)">
                  <i
                    v-if="!isSprintNotVisible[sprint.id]"
                    class="fa-solid fa-chevron-down ml-1 text-xs mr-2"
                  ></i>
                  <i
                    v-else
                    class="fa-solid fa-chevron-right ml-1 text-xs mr-2"
                  ></i>
                </button>
...
onMounted(async () => {
  try {
    ...
    const requestPayload: SprintProjectRequest = {
      status: statusSprintSearch.value,
    };
    const sprintResponse = await fetchSprintProject(statusSprintSearch.value);
    console.log("(sprintResponse)", sprintResponse);
    const filteredSprints = (sprintResponse.data as Sprint[]).filter(
      (sprint) => sprint.status !== SprintStatus.COMPLETE
    );
