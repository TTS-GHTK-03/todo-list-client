<template>
  ...
  <div v-else>
    <div class="mt-24 px-8 py-0">
      <div class="min-h-[130px] flex flex-col justify-around">
        ...
        <div class="flex mb-0">
          ...
          <div class="flex items-center mr-6">
            ...
            <a-button shape="circle" @click="openModal"
              class="bg-gray-100 text-sm text-text-dark-thin hover:bg-gray-200 rounded h-9 w-9 flex px-2 items-center justify-center ml-2">
      ...
      <div class="flex gap-6 mt-8 h-[450px] overflow-y-auto">
        <div class="w-[260px] min-h-[180px] bg-gray-100 bg-opacity-70 rounded flex flex-col"
          ...
          <div class="mt-0 m-1 flex flex-col items-center min-h-[100px]">
            <item-task v-for="task in data.get(TaskStatus.TODO)" :key="task?.id" :text="task?.title"
              :keyText="task?.keyProjectTask" :tooltip-title="task?.title" draggable="true"
        ...
        <div class="w-[260px] min-h-[180px] bg-gray-100 bg-opacity-70 rounded flex flex-col"
          ...
          <!-- Container for item-task, positioned right under the header -->
          <div class="mt-0 m-1 flex flex-col items-center min-h-[100px]">
            <item-task v-for="task in data.get(TaskStatus.IN_PROGRESS)" :key="task?.id" :text="task?.title"
              :keyText="task?.keyProjectTask" :tooltip-title="task?.title" draggable="true"
              @dragstart="startDrag($event, task)" />
          </div>
        </div>
        ...
        <div class="w-[260px] min-h-[180px] bg-gray-100 bg-opacity-70 rounded flex flex-col"
          ...
          <!-- Container for item-task, positioned right under the header -->
          <div class="mt-0 m-1 flex flex-col items-center min-h-[100px]">
            <item-task v-for="task in data.get(TaskStatus.READY_FOR_TEST)" :key="task?.id" :text="task?.title"
              :keyText="task?.keyProjectTask" :tooltip-title="task?.title" draggable="true"
              @dragstart="startDrag($event, task)" />
          </div>
        </div>
        ...
        <div class="w-[260px] min-h-[180px] bg-gray-100 bg-opacity-70 rounded flex flex-col"
          ...
          <!-- Container for item-task, positioned right under the header -->
          <div class="mt-0 m-1 flex flex-col items-center min-h-[100px]">
            <item-task v-for="task in data.get(TaskStatus.DONE)" :key="task?.id" :text="task?.title"
              :keyText="task?.keyProjectTask" :tooltip-title="task?.title" draggable="true"
              @dragstart="startDrag($event, task)" />
          </div>
        </div>
