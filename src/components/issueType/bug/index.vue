      ...
      <div class="p-4 flex flex-col mx-72">
        ...
        <div class="flex items-center justify-between mt-24">
          ...
          <div class="flex items-center flex-grow mt-0">
            ...
            <!-- Phần tiêu đề -->
            <div class="flex items-center justify-between ml-2 relative flex-grow h-11 transition-all duration-300">
              <div v-if="!editingTitle" @click="startEditing('title')"
          ...
          <div
            ...
            <AddLabelModal :typeId="currentType.id" :visible="isModalVisible" @addNewLabel="handleCreateLabel"
        ...
        <div class="flex items-center justify-between mt-4">
          <div v-if="!editingDescription" @click="startEditing('description')"
            class="text-slate-900 px-3 pt-2 pb-2 ml-2 info my-1.5 w-full h-10 cursor-pointer hover:bg-gray-100 hover:bg-opacity-80">
            {{ description }}
          </div>
          <div v-else class="relative flex items-center w-full">
            <input v-model="description" spellcheck="false" type="text"
              class="border-2 border-blue-500 rounded w-full h-10 text-sm focus:border-blue-500 pl-2"
              @blur="stopEditing" ref="descriptionInput"
              style="border-color: #3b82f6 !important; outline: none !important" />
            <div class="absolute right-0 top-full mt-1 flex space-x-1">
              <button @click="confirmEdit('description')"
                class="h-8 w-8 rounded border bg-white hover:bg-gray-200 shadow">
                <i class="fa-solid fa-check"></i>
              </button>
        ...
        <div class="mt-6 h-[400px] overflow-y-auto">
          <div v-for="item in listItems" :key="item.id" class="flex items-center p-4 border-b hover:bg-gray-100">
            <input type="checkbox" class="mr-4 h-5 w-5 text-blue-500 border-gray-300 rounded" />
            <div>
...
async function confirmEdit(field: any) {
  if (field === "description") {
  } else if (field === "title") {
  }
  stopEditing();
...
async function fetchData() {
  ...
  currentType.value = allType.find((type: any) => type.title === title.value);
  loadAllLabel();
  }catch(e){
    console.log(e);
  } finally {
