<template>
    <div class="mt-20 px-10 py-0 ml-4 relative">
        ...
        <div>
            <a-table :row-selection="rowSelection" :columns="columns" :data-source="data" :customRow="customRow"
                ...
                <template #bodyCell="{ column, text, record }">
                    ...
                    <template v-if="column.dataIndex === 'lead'">
                        ...
                        >
                            ...
                            <!-- Hover Box -->
                            <div v-if="activeLead === record.lead" class="cursor-default absolute top-[-210px] left-0 w-[360px] h-[200px] bg-white border border-gray-300 shadow-lg z-50 flex flex-col  rounded-lg">
                                <div class="absolute top-[30px] left-[40px] w-[100px] h-[100px] rounded-full bg-[#1b2b4e] text-white text-5xl flex items-center justify-center">
                                    {{ text.charAt(0) }}
                                </div>

                                <div class="w-full h-[100px] bg-blue-600 flex items-center justify-center border-t rounded">
                                    <div class="mt-8 ml-20 text-white font-apple text-lg">{{ text }}</div>
                                </div>
                                ...
                                <div class="w-full h-[100px] bg-white flex items-center justify-center flex-col">
                                    ...
                                    <button class="mb-4 ml-48 bg-gray-200 hover p-2 text-[#172B4D] font-apple font-semibold border rounded">View profile</button>
                                </div>
                            </div>
                        </div>
                    </template>
                    <template v-if="column.dataIndex === 'actions'">
                        <div class="flex items-center justify-center h-10 w-10 cursor-pointer bg-white rounded hover:bg-gray-100 ml-8">
    ...
    setup() {
        ...
        const toggleHover = (lead: string) => {
            if (activeLead.value === lead) {
                activeLead.value = null; // Hide the box if it is already active
            } else {
                activeLead.value = lead; // Show the box for the clicked lead
            }
        };
        ...
        const customRow = (record: DataType) => {
            return {
                onMouseover: (event: MouseEvent) => {
                    (event.currentTarget as HTMLElement).classList.add('custom-row-hover');
                },
                onMouseout: (event: MouseEvent) => {
                    (event.currentTarget as HTMLElement).classList.remove('custom-row-hover');
                }
            };
        };
